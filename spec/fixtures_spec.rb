...
RSpec.describe 'Fixtures' do
    ...
    FixtureHelper::FIXTURES.each do |fixture_name, model|
      it "#{fixture_name} fixture count matches database count" do
        ...
        db_count = if model
                     ...
                     ActiveRecord::Base.connection.select_value("SELECT COUNT(*) FROM #{fixture_name}").to_i
                   end
  ...
  describe 'fixture files' do
    FixtureHelper::FIXTURES.each do |fixture_model_pair|
      fixture_name, _model = *fixture_model_pair
      it "#{fixture_name}.yml has no duplicate keys" do
        ...
        lines.each do |line|
          ...
          keys << Regexp.last_match(1) if in_document && line =~ /^([a-zA-Z0-9_]+):/
        end
  ...
  describe 'record validity' do
    FixtureHelper::FIXTURES.each do |fixture_model_pair|
      _fixture_name, model = *fixture_model_pair
  ...
  describe 'join table foreign key validity' do
    join_tables = FixtureHelper::FIXTURES.select { |fixture_model_pair| fixture_model_pair.last.nil? }.map(&:first)
  ...
  describe 'non-null field coverage' do
    FixtureHelper::FIXTURES.each do |fixture_model_pair|
      ...
      it "#{model.name} fixtures include examples of all nullable fields" do
        ...
        nullable_columns = columns.select(&:null)
