...
RSpec.describe PullRequestsImporter do
  ...
  describe '#import', vcr: { cassette_name: 'pull_requests_importer/import' } do
    before do
      # Create test data
      user = FixtureHelper.find(GithubUser, :ianheggie_oaf) || 
        GithubUser.create!(
          id: 1234567,
          login: 'ianheggie-oaf',
      ...
      scraper = FixtureHelper.find(Scraper, :multiple_masterview) ||
        Scraper.create!(
    ...
    it 'associates PRs with authorities when possible' do
      authority = FixtureHelper.find(Authority, :brimbank) ||
        Authority.create!(
          short_name: 'brimbank',
  ...
  describe '#guess_associated_authorities' do
    ...
    let(:authority) { FixtureHelper.find(Authority, :brimbank) || Authority.create!(short_name: 'brimbank', scraper: scraper) }
