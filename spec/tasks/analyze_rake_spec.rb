...
RSpec.describe 'analyze rake tasks' do
  ...
  describe 'analyze:all' do
    it 'has the correct prerequisites' do
      task = Rake::Task['analyze:all']
      expect(task.prerequisite_tasks.map(&:name)).to match_array(%w[singleton analyze:broken_scores])
    end
  end
